...
class UartReceiver(Node):
    ...
    def __init__(self, name, serial_port, baudrate, topic_stack=10):
        ...
        self.pub = self.create_publisher(String, name + "_topic",  topic_stack)    # 创建发布方，发布数据到 uart1_receiver_topic 
    ...
    def publish(self, data):
        ...
        self.get_logger().info(f'接收数据发布到 uart1_receiver_topic | 数据：{data}')


def main(args=None):
    rclpy.init(args=args)
    uartReceiver = UartReceiver("uart1_receiver", "/dev/ttyS1", 115200)
    uartReceiver.get_logger().info(f"uart1_receiver @ {uartReceiver.serial_port} Buad: {uartReceiver.baudrate} init success")
...
# 其他节点 接收信息 回调函数
    def uart1_receiver_init(self, topic_stack=10):
        '''
            ...
            【需搭配定时器进行使用】
        '''
        self.sub_uart1_receiver = self.create_subscription(String, "uart1_receiver_topic",self.uart1_receiver_callback, topic_stack)
        # 有数据发布在 uart1_receiver_topic 话题，即调用uart1_receiver_callback

    def uart1_receiver_callback(self, msg):
        '''
            在非串口节点中使用
            订阅 uart1_receiver_topic 话题，接收相关数据进行处理
