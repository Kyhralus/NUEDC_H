...
def main(args=None):
    ...
    uartSender = UartSender("uart0_sender", "/dev/ttyS0", 115200)
    uartSender.get_logger().info(f"uart0_sender @ {uartSender.serial_port} Buad: {uartSender.baudrate} init success")
...
# ----------------------------------------------
# 其他节点 发送信息 函数
def uart0_sender_init(self, topic_stack=10):
    '''
        在其他话题的初始化中使用
        创建一个发布方 ---- 发布方功能：发送数据给串口 
        功能：发送数据到 uart_sender_topic
    '''
    self.pub_uart0_sender = self.create_publisher(String, 'uart0_sender_topic', topic_stack)
    self.get_logger().info(f'创建uart0_sender_topic发布方成功！')  

def uart0_sender_send(self, data):
    '''
        在非串口话题中使用
    ...
    msg = String()
    msg.data = data
    self.pub_uart0_sender.publish(msg)
